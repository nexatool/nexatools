...
// ---------- RESUME BUILDER ----------
function initResume() {
  const accents = ['#bf5af2', '#00e5ff', '#30d158', '#ff453a', '#ffd60a', '#6a9fff', '#ff6b9d', '#ff9f43', '#a29bfe'];
...
function setResumeTemplate(tpl, el) {
  ...
  if (accentMap[tpl] && !document.querySelector('#r-accent-row .swatch.active')) {
    resumeAccent = accentMap[tpl];
  }
  buildResume();
}
...
function buildResume() {
  ...
  const fontStyleRaw = $('r-font-style')?.value || 'normal';
  const fontWeight  = fontStyleRaw.includes('bold')   ? 'bold'   : 'normal';
  const fontStyle   = fontStyleRaw.includes('italic') ? 'italic' : 'normal';
  const fontCSS     = `font-family:'${fontFamily}',serif;font-weight:${fontWeight};font-style:${fontStyle};`;
  ...
  // ==================== CLASSIC — Clean Professional (FREE) ====================
  if (tpl === 'classic') {
    html = `
    <div style="${fontCSS}background:#fff;color:#1a1a2e;min-height:500px;">
      <div style="background:${acc};padding:36px 40px;color:#fff;position:relative;overflow:hidden;">
        ...
        <div style="position:absolute;right:60px;bottom:-30px;width:80px;height:80px;background:rgba(255,255,255,0.05);border-radius:50%;"></div>
        <div style="font-size:2.2rem;font-weight:800;letter-spacing:-0.5px;position:relative;">${name}</div>
        <div style="font-size:1rem;opacity:0.92;margin-top:5px;font-weight:500;letter-spacing:1px;text-transform:uppercase;font-size:0.85rem;position:relative;">${title}</div>
        <div style="display:flex;gap:20px;margin-top:12px;font-size:0.78rem;opacity:0.85;flex-wrap:wrap;position:relative;">${contactParts.map(c=>`<span>● ${c}</span>`).join('')}</div>
        ${socialLinks ? `<div style="font-size:0.74rem;margin-top:6px;opacity:0.8;position:relative;">${socialLinks}</div>` : ''}
      </div>
      <div style="padding:32px 40px;">
        ${about ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin-bottom:10px;">Profile Summary</div><p style="font-size:0.88rem;line-height:1.8;color:#444;margin-bottom:24px;">${about}</p>` : ''}
        ${skills ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin-bottom:12px;">Skills</div><div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px;">${skills.split(',').map(s=>`<span style="background:${acc}18;border:1px solid ${acc}55;color:${acc === '#ffffff' ? '#333' : acc};padding:5px 14px;border-radius:20px;font-size:0.78rem;font-weight:600;">${s.trim()}</span>`).join('')}</div>` : ''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;">
          <div>
            ${exp ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin-bottom:10px;">Work Experience</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${exp}</p>` : ''}
            ${proj ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin:20px 0 10px;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
          </div>
          <div>
            ${edu ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin-bottom:10px;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${edu}</p>` : ''}
            ${lang ? `<div style="font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};border-left:4px solid ${acc};padding-left:12px;margin:20px 0 10px;">Languages</div><p style="font-size:0.85rem;line-height:1.8;color:#555;">${lang}</p>` : ''}
          </div>
        </div>
      </div>
    </div>`;

  // ==================== MODERN — Two-Column Dark Header ====================
  } else if (tpl === 'modern') {
    html = `
    <div style="${fontCSS}background:#fff;color:#222;min-height:500px;">
      <div style="background:linear-gradient(135deg,#0d0d1a 0%,#1a1028 50%,${acc}33 100%);padding:40px;color:#fff;position:relative;overflow:hidden;">
        <div style="position:absolute;right:-40px;top:-40px;width:200px;height:200px;background:${acc}15;border-radius:50%;"></div>
        <div style="position:absolute;right:80px;bottom:-60px;width:140px;height:140px;background:${acc}10;border-radius:50%;"></div>
        <div style="display:flex;align-items:center;gap:24px;position:relative;">
          <div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,${acc},${acc}88);display:flex;align-items:center;justify-content:center;font-size:1.8rem;font-weight:900;color:#fff;flex-shrink:0;border:3px solid rgba(255,255,255,0.2);">${initials}</div>
          <div>
            <div style="font-size:2rem;font-weight:900;letter-spacing:-1px;">${name}</div>
            <div style="font-size:0.88rem;color:${acc};margin-top:4px;font-weight:600;letter-spacing:1px;">${title}</div>
            <div style="display:flex;gap:16px;margin-top:10px;font-size:0.75rem;opacity:0.75;flex-wrap:wrap;">${contactParts.map(c=>`<span>${c}</span>`).join('<span style="opacity:0.4">|</span>')}</div>
            ${socialLinks ? `<div style="font-size:0.72rem;margin-top:5px;opacity:0.7;">${socialLinks}</div>` : ''}
          </div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:200px 1fr;">
        <div style="background:#f5f7ff;padding:28px 20px;border-right:1px solid #eee;">
          ${skills ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">Skills</div>${skills.split(',').map(s=>`<div style="padding:7px 0;border-bottom:1px solid #ececf5;font-size:0.8rem;color:#333;display:flex;align-items:center;gap:6px;"><span style="width:6px;height:6px;border-radius:50%;background:${acc};flex-shrink:0;display:inline-block;"></span>${s.trim()}</div>`).join('')}` : ''}
          ${edu ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin:20px 0 10px;">Education</div><p style="font-size:0.78rem;line-height:1.7;color:#555;white-space:pre-wrap;">${edu}</p>` : ''}
          ${lang ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin:20px 0 10px;">Languages</div>${lang.split(',').map(l=>`<div style="padding:5px 0;font-size:0.78rem;color:#555;">◈ ${l.trim()}</div>`).join('')}` : ''}
        </div>
        <div style="padding:28px 32px;">
          ${about ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid ${acc};">About Me</div><p style="font-size:0.88rem;line-height:1.8;color:#444;margin-bottom:24px;">${about}</p>` : ''}
          ${exp ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid ${acc};">Work Experience</div><p style="font-size:0.88rem;line-height:1.8;color:#333;margin-bottom:24px;white-space:pre-wrap;">${exp}</p>` : ''}
          ${proj ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid ${acc};">Projects</div><p style="font-size:0.88rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
        </div>
      </div>
    </div>`;

  // ==================== MINIMAL — Swiss Clean Style ====================
  } else if (tpl === 'minimal') {
    html = `
    <div style="${fontCSS}background:#fafafa;color:#111;min-height:500px;padding:48px 52px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-end;padding-bottom:24px;margin-bottom:32px;border-bottom:2px solid #111;">
        <div>
          <div style="font-size:2.6rem;font-weight:300;letter-spacing:-1.5px;line-height:1;color:#0a0a0a;">${name}</div>
          <div style="font-size:0.9rem;color:${acc};margin-top:8px;font-weight:600;letter-spacing:2px;text-transform:uppercase;">${title}</div>
        </div>
        <div style="text-align:right;">
          ${contactParts.map(c=>`<div style="font-size:0.78rem;color:#666;line-height:1.9;">${c}</div>`).join('')}
          ${socialLinks ? `<div style="font-size:0.72rem;color:#888;margin-top:4px;">${socialLinks}</div>` : ''}
        </div>
      </div>
      ${about ? `<p style="font-size:0.92rem;line-height:1.9;color:#444;margin-bottom:36px;max-width:620px;">${about}</p>` : ''}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;">
        <div>
          ${exp ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#999;margin-bottom:16px;">Experience</div>${exp.split('\n').filter(Boolean).map(line=>`<div style="padding:10px 0;border-bottom:1px solid #eee;font-size:0.85rem;color:#333;">${line}</div>`).join('')}` : ''}
          ${proj ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#999;margin:24px 0 16px;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
        </div>
        <div>
          ${skills ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#999;margin-bottom:16px;">Skills</div><div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px;">${skills.split(',').map(s=>`<span style="border:1px solid #ddd;padding:4px 12px;border-radius:3px;font-size:0.8rem;color:#333;">${s.trim()}</span>`).join('')}</div>` : ''}
          ${edu ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#999;margin-bottom:16px;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${edu}</p>` : ''}
          ${lang ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#999;margin:24px 0 16px;">Languages</div>${lang.split(',').map(l=>`<div style="padding:6px 0;font-size:0.85rem;color:#333;border-bottom:1px solid #f0f0f0;">${l.trim()}</div>`).join('')}` : ''}
        </div>
      </div>
    </div>`;

  // ==================== BOLD — Dark Impact Style ====================
  } else if (tpl === 'bold') {
    html = `
    <div style="${fontCSS}background:#080810;color:#fff;min-height:500px;">
      <div style="padding:44px 40px 36px;border-bottom:3px solid ${acc};">
        <div style="font-size:0.65rem;color:${acc};letter-spacing:4px;text-transform:uppercase;margin-bottom:12px;">● CURRICULUM VITAE</div>
        <div style="font-size:3rem;font-weight:900;text-transform:uppercase;letter-spacing:-2px;line-height:0.95;color:#fff;">${name.split(' ')[0]}</div>
        <div style="font-size:3rem;font-weight:900;text-transform:uppercase;letter-spacing:-2px;line-height:0.95;color:${acc};">${name.split(' ').slice(1).join(' ')}</div>
        <div style="font-size:0.88rem;color:#888;margin-top:16px;font-weight:400;letter-spacing:3px;text-transform:uppercase;">${title}</div>
        <div style="display:flex;gap:24px;margin-top:14px;font-size:0.75rem;color:#666;">${contactParts.map(c=>`<span>${c}</span>`).join('')}</div>
        ${socialLinks ? `<div style="font-size:0.72rem;color:#555;margin-top:6px;">${socialLinks}</div>` : ''}
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;">
        <div style="padding:32px 36px;border-right:1px solid #1a1a2e;">
          ${about ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">About</div><p style="font-size:0.85rem;line-height:1.8;color:#bbb;margin-bottom:28px;">${about}</p>` : ''}
          ${skills ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Skills</div><div style="display:flex;flex-direction:column;gap:8px;margin-bottom:28px;">${skills.split(',').map(s=>`<div style="display:flex;align-items:center;justify-content:space-between;"><span style="font-size:0.82rem;color:#ccc;">${s.trim()}</span><div style="width:80px;height:3px;background:#1a1a2e;border-radius:2px;"><div style="width:85%;height:100%;background:${acc};border-radius:2px;"></div></div></div>`).join('')}</div>` : ''}
          ${lang ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">Languages</div><p style="font-size:0.85rem;color:#bbb;">${lang}</p>` : ''}
        </div>
        <div style="padding:32px 36px;">
          ${exp ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Experience</div><p style="font-size:0.85rem;line-height:1.8;color:#bbb;white-space:pre-wrap;margin-bottom:28px;">${exp}</p>` : ''}
          ${edu ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#bbb;white-space:pre-wrap;margin-bottom:28px;">${edu}</p>` : ''}
          ${proj ? `<div style="font-size:0.6rem;font-weight:900;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#bbb;white-space:pre-wrap;">${proj}</p>` : ''}
        </div>
      </div>
    </div>`;

  // ==================== EXECUTIVE — Luxury Centered ====================
  } else if (tpl === 'executive') {
    html = `
    <div style="${fontCSS}background:#fff;color:#222;min-height:500px;">
      <div style="background:linear-gradient(180deg,#0f0f1e 0%,#1a1a30 100%);padding:48px 40px;text-align:center;">
        <div style="width:80px;height:2px;background:linear-gradient(90deg,transparent,${acc},transparent);margin:0 auto 28px;"></div>
        <div style="width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,${acc}88,${acc});display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:900;color:#fff;margin:0 auto 24px;border:2px solid ${acc}44;">${initials}</div>
        <div style="font-size:2.4rem;font-weight:700;color:#fff;letter-spacing:3px;text-transform:uppercase;font-family:Georgia,serif;">${name}</div>
        <div style="font-size:0.78rem;color:${acc};margin-top:10px;letter-spacing:6px;text-transform:uppercase;">${title}</div>
        <div style="width:80px;height:2px;background:linear-gradient(90deg,transparent,${acc},transparent);margin:24px auto 20px;"></div>
        <div style="display:flex;justify-content:center;gap:24px;font-size:0.75rem;color:#888;flex-wrap:wrap;">${contactParts.map(c=>`<span>${c}</span>`).join('<span style="color:#333;">·</span>')}</div>
        ${socialLinks ? `<div style="font-size:0.72rem;color:#666;margin-top:8px;">${socialLinks}</div>` : ''}
      </div>
      <div style="padding:40px 52px;">
        ${about ? `<p style="font-size:0.95rem;line-height:2;color:#555;text-align:center;font-style:italic;border:1px solid #f0f0f0;padding:24px 32px;border-radius:8px;margin-bottom:36px;background:#fafafa;">"${about}"</p>` : ''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;">
          <div>
            ${skills ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:${acc};margin-bottom:16px;padding-bottom:8px;border-bottom:2px solid ${acc}33;">Core Competencies</div><div style="display:flex;flex-direction:column;gap:0;">${skills.split(',').map(s=>`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5;font-size:0.85rem;"><span style="color:${acc};font-size:0.7rem;">◆</span>${s.trim()}</div>`).join('')}</div>` : ''}
            ${lang ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:${acc};margin:24px 0 14px;padding-bottom:8px;border-bottom:2px solid ${acc}33;">Languages</div><p style="font-size:0.85rem;line-height:1.9;color:#555;">${lang}</p>` : ''}
          </div>
          <div>
            ${exp ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:${acc};margin-bottom:16px;padding-bottom:8px;border-bottom:2px solid ${acc}33;">Professional Experience</div><p style="font-size:0.85rem;line-height:1.9;color:#333;white-space:pre-wrap;">${exp}</p>` : ''}
            ${edu ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:${acc};margin:24px 0 14px;padding-bottom:8px;border-bottom:2px solid ${acc}33;">Education</div><p style="font-size:0.85rem;line-height:1.9;color:#333;white-space:pre-wrap;">${edu}</p>` : ''}
          </div>
        </div>
        ${proj ? `<div style="margin-top:32px;padding-top:28px;border-top:1px solid #eee;"><div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:${acc};margin-bottom:14px;">Notable Projects</div><p style="font-size:0.85rem;line-height:1.9;color:#333;white-space:pre-wrap;">${proj}</p></div>` : ''}
      </div>
    </div>`;

  // ==================== CREATIVE — Photo Sidebar Card ====================
  } else if (tpl === 'creative') {
    html = `
    <div style="${fontCSS}background:#fff;display:grid;grid-template-columns:240px 1fr;min-height:550px;">
      <div style="background:linear-gradient(180deg,#0f0f1e 0%,${acc}22 100%);padding:36px 24px;color:#fff;display:flex;flex-direction:column;gap:0;">
        <div style="width:90px;height:90px;background:linear-gradient(135deg,${acc},${acc}88);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2.4rem;font-weight:900;color:#fff;margin:0 auto 20px;border:3px solid ${acc}44;">${initials}</div>
        <div style="font-size:1.3rem;font-weight:800;text-align:center;line-height:1.2;">${name}</div>
        <div style="font-size:0.75rem;color:${acc};margin-top:6px;text-align:center;letter-spacing:1px;">${title}</div>
        <div style="height:1px;background:linear-gradient(90deg,transparent,${acc}66,transparent);margin:20px 0;"></div>
        <div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">Contact</div>
        ${contactParts.map(c=>`<div style="font-size:0.76rem;color:#aaa;margin-bottom:8px;display:flex;align-items:flex-start;gap:6px;line-height:1.4;"><span style="color:${acc};margin-top:2px;">▸</span>${c}</div>`).join('')}
        ${socialLinks ? `<div style="font-size:0.72rem;color:#888;margin-top:4px;word-break:break-all;">${socialLinks}</div>` : ''}
        ${skills ? `<div style="height:1px;background:linear-gradient(90deg,transparent,${acc}66,transparent);margin:20px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Skills</div>${skills.split(',').map(s=>`<div style="margin-bottom:10px;"><div style="font-size:0.76rem;color:#ccc;margin-bottom:5px;">${s.trim()}</div><div style="height:3px;background:rgba(255,255,255,0.08);border-radius:2px;overflow:hidden;"><div style="height:100%;width:82%;background:linear-gradient(90deg,${acc},${acc}88);border-radius:2px;"></div></div></div>`).join('')}` : ''}
        ${lang ? `<div style="height:1px;background:linear-gradient(90deg,transparent,${acc}66,transparent);margin:16px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">Languages</div>${lang.split(',').map(l=>`<div style="font-size:0.76rem;color:#aaa;margin-bottom:6px;">◈ ${l.trim()}</div>`).join('')}` : ''}
      </div>
      <div style="padding:36px 36px;">
        ${about ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">About Me</div><p style="font-size:0.88rem;line-height:1.8;color:#444;margin-bottom:28px;padding-bottom:22px;border-bottom:1px solid #eee;">${about}</p>` : ''}
        ${exp ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Work Experience</div>${exp.split('\n').filter(Boolean).map((line,i)=>`<div style="padding:10px 0;border-bottom:1px solid #f5f5f5;"><div style="font-size:0.85rem;color:#222;">${line}</div></div>`).join('')}<div style="margin-bottom:24px;"></div>` : ''}
        ${edu ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:24px;">${edu}</p>` : ''}
        ${proj ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
      </div>
    </div>`;

  // ==================== NEON DARK — Developer Terminal ====================
  } else if (tpl === 'neon') {
    html = `
    <div style="${fontCSS}background:#020209;color:#fff;min-height:500px;padding:36px;">
      <div style="border:1px solid ${acc}55;border-radius:16px;padding:32px;margin-bottom:28px;position:relative;background:rgba(255,255,255,0.02);">
        <div style="position:absolute;top:-1px;left:30px;right:30px;height:2px;background:linear-gradient(90deg,transparent,${acc},transparent);"></div>
        <div style="position:absolute;bottom:-1px;left:30px;right:30px;height:1px;background:linear-gradient(90deg,transparent,${acc}55,transparent);"></div>
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
          <div>
            <div style="font-size:0.6rem;color:${acc}88;letter-spacing:3px;margin-bottom:8px;">// RESUME.md</div>
            <div style="font-size:2.4rem;font-weight:700;color:#fff;text-shadow:0 0 30px ${acc}66;letter-spacing:-1px;">${name}</div>
            <div style="font-size:0.88rem;color:${acc};margin-top:6px;text-shadow:0 0 12px ${acc}55;">${title}</div>
          </div>
          <div style="text-align:right;">
            ${contactParts.map(c=>`<div style="font-size:0.75rem;color:#666;margin-bottom:5px;">${c}</div>`).join('')}
            ${socialLinks ? `<div style="font-size:0.72rem;color:#555;margin-top:6px;">${socialLinks}</div>` : ''}
          </div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;">
        <div>
          ${about ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;margin-bottom:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">// about</div><p style="font-size:0.83rem;line-height:1.8;color:#9999bb;">${about}</p></div>` : ''}
          ${skills ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;margin-bottom:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:12px;">// skills</div><div style="display:flex;flex-wrap:wrap;gap:7px;">${skills.split(',').map(s=>`<span style="background:${acc}15;border:1px solid ${acc}44;color:${acc};padding:4px 11px;border-radius:5px;font-size:0.76rem;">${s.trim()}</span>`).join('')}</div></div>` : ''}
          ${lang ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">// languages</div><p style="font-size:0.83rem;color:#9999bb;">${lang}</p></div>` : ''}
        </div>
        <div>
          ${exp ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;margin-bottom:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">// experience</div><p style="font-size:0.83rem;line-height:1.8;color:#9999bb;white-space:pre-wrap;">${exp}</p></div>` : ''}
          ${edu ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;margin-bottom:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">// education</div><p style="font-size:0.83rem;line-height:1.8;color:#9999bb;white-space:pre-wrap;">${edu}</p></div>` : ''}
          ${proj ? `<div style="background:rgba(255,255,255,0.02);border:1px solid ${acc}22;border-radius:10px;padding:20px;"><div style="font-size:0.55rem;color:${acc};letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">// projects</div><p style="font-size:0.83rem;line-height:1.8;color:#9999bb;white-space:pre-wrap;">${proj}</p></div>` : ''}
        </div>
      </div>
      <div style="margin-top:20px;font-size:0.62rem;color:#222;text-align:right;font-family:monospace;">nexatools.io ∎</div>
    </div>`;
    // ==================== PHOTO LEFT — Dark Sidebar with Avatar ====================
  } else if (tpl === 'photoleft') {
    html = `
    <div style="${fontCSS}background:#fff;display:grid;grid-template-columns:200px 1fr;min-height:550px;">
      <div style="background:linear-gradient(180deg,#0f0f1e 0%,#1e0f2e 100%);padding:32px 20px;color:#fff;display:flex;flex-direction:column;gap:0;">
        <div style="width:80px;height:80px;background:linear-gradient(135deg,${acc},${acc}88);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:900;color:#fff;margin:0 auto 16px;box-shadow:0 0 0 4px ${acc}33;">${initials}</div>
        <div style="font-size:1.2rem;font-weight:800;text-align:center;line-height:1.2;margin-bottom:4px;">${name}</div>
        <div style="font-size:0.72rem;color:${acc};text-align:center;letter-spacing:1px;margin-bottom:20px;">${title}</div>
        <div style="height:1px;background:${acc}44;margin-bottom:16px;"></div>
        <div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">Contact</div>
        ${contactParts.map(c=>`<div style="font-size:0.75rem;color:#bbb;margin-bottom:7px;line-height:1.4;">▸ ${c}</div>`).join('')}
        ${socialLinks ? `<div style="font-size:0.7rem;color:#888;margin-top:6px;word-break:break-all;">${socialLinks}</div>` : ''}
        ${skills ? `<div style="height:1px;background:${acc}44;margin:16px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">Skills</div>${skills.split(',').map(s=>`<div style="margin-bottom:9px;"><div style="font-size:0.74rem;color:#ccc;margin-bottom:4px;">${s.trim()}</div><div style="height:3px;background:rgba(255,255,255,0.08);border-radius:2px;"><div style="height:100%;width:80%;background:linear-gradient(90deg,${acc},${acc}88);border-radius:2px;"></div></div></div>`).join('')}` : ''}
        ${lang ? `<div style="height:1px;background:${acc}44;margin:16px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">Languages</div>${lang.split(',').map(l=>`<div style="font-size:0.74rem;color:#aaa;margin-bottom:5px;">◈ ${l.trim()}</div>`).join('')}` : ''}
      </div>
      <div style="padding:32px 36px;background:#fff;">
        ${about ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};padding-bottom:6px;border-bottom:2px solid ${acc};margin-bottom:12px;">Profile</div><p style="font-size:0.87rem;line-height:1.8;color:#444;margin-bottom:24px;">${about}</p>` : ''}
        ${exp ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};padding-bottom:6px;border-bottom:2px solid ${acc};margin-bottom:12px;">Experience</div><p style="font-size:0.87rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:24px;">${exp}</p>` : ''}
        ${edu ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};padding-bottom:6px;border-bottom:2px solid ${acc};margin-bottom:12px;">Education</div><p style="font-size:0.87rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:24px;">${edu}</p>` : ''}
        ${proj ? `<div style="font-size:0.58rem;font-weight:800;text-transform:uppercase;letter-spacing:3px;color:${acc};padding-bottom:6px;border-bottom:2px solid ${acc};margin-bottom:12px;">Projects</div><p style="font-size:0.87rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
      </div>
    </div>`;
  ...
  // ==================== TWO COLUMN — Split Layout ====================
    ...
    html = `
    <div style="${fontCSS}background:#fff;color:#1a1a2e;min-height:550px;">
      <div style="background:#1a1a2e;padding:32px 40px;display:flex;align-items:center;gap:24px;">
        <div style="width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,${acc},${acc}66);display:flex;align-items:center;justify-content:center;font-size:1.8rem;font-weight:900;color:#fff;flex-shrink:0;">${initials}</div>
        <div>
          <div style="font-size:1.9rem;font-weight:800;color:#fff;letter-spacing:-0.5px;">${name}</div>
          <div style="font-size:0.82rem;color:${acc};margin-top:4px;letter-spacing:2px;text-transform:uppercase;">${title}</div>
          <div style="display:flex;gap:16px;margin-top:8px;font-size:0.74rem;color:#888;flex-wrap:wrap;">${contactParts.join(' · ')}</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;min-height:400px;">
        <div style="padding:28px 32px;border-right:1px solid #eee;">
          ${about ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">About Me</div><p style="font-size:0.85rem;line-height:1.8;color:#444;margin-bottom:22px;">${about}</p>` : ''}
          ${skills ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">Skills</div><div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:22px;">${skills.split(',').map(s=>`<span style="background:${acc}15;border:1px solid ${acc}44;color:${acc === '#ffffff'?'#333':acc};padding:4px 12px;border-radius:20px;font-size:0.77rem;">${s.trim()}</span>`).join('')}</div>` : ''}
          ${lang ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">Languages</div>${lang.split(',').map(l=>`<div style="padding:5px 0;font-size:0.83rem;color:#444;border-bottom:1px solid #f5f5f5;">◈ ${l.trim()}</div>`).join('')}` : ''}
        </div>
        <div style="padding:28px 32px;">
          ${exp ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">Work Experience</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:22px;">${exp}</p>` : ''}
          ${edu ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:22px;">${edu}</p>` : ''}
          ${proj ? `<div style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:5px;border-bottom:2px solid ${acc}22;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
        </div>
      </div>
    </div>`;

  // ==================== INFOGRAPHIC — Icon Dots Dark ====================
  } else if (tpl === 'infographic') {
    html = `
    <div style="${fontCSS}background:#08080f;color:#fff;min-height:550px;">
      <div style="background:linear-gradient(90deg,${acc},${acc}88);padding:28px 40px;display:flex;align-items:center;gap:20px;">
        <div style="width:64px;height:64px;border-radius:50%;background:rgba(0,0,0,0.3);border:3px solid rgba(255,255,255,0.4);display:flex;align-items:center;justify-content:center;font-size:1.6rem;font-weight:900;flex-shrink:0;">${initials}</div>
        <div>
          <div style="font-size:1.9rem;font-weight:900;color:#fff;letter-spacing:-0.5px;">${name}</div>
          <div style="font-size:0.82rem;color:rgba(255,255,255,0.8);margin-top:4px;letter-spacing:2px;text-transform:uppercase;">${title}</div>
          <div style="display:flex;gap:14px;margin-top:8px;font-size:0.73rem;color:rgba(255,255,255,0.7);flex-wrap:wrap;">${contactParts.join(' · ')}</div>
          ${socialLinks ? `<div style="font-size:0.7rem;color:rgba(255,255,255,0.6);margin-top:4px;">${socialLinks}</div>` : ''}
        </div>
      </div>
      <div style="display:grid;grid-template-columns:220px 1fr;">
        <div style="padding:24px 20px;border-right:1px solid rgba(255,255,255,0.07);">
          ${skills ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:14px;">● Skills</div>${skills.split(',').map(s=>`<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.05);"><div style="width:7px;height:7px;border-radius:50%;background:${acc};flex-shrink:0;"></div><span style="font-size:0.8rem;color:#ccc;">${s.trim()}</span></div>`).join('')}` : ''}
          ${lang ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin:18px 0 12px;">● Languages</div>${lang.split(',').map(l=>`<div style="font-size:0.8rem;color:#aaa;margin-bottom:6px;">◈ ${l.trim()}</div>`).join('')}` : ''}
        </div>
        <div style="padding:24px 28px;">
          ${about ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;">● About</div><p style="font-size:0.85rem;line-height:1.8;color:#aaa;margin-bottom:22px;padding-bottom:18px;border-bottom:1px solid rgba(255,255,255,0.07);">${about}</p>` : ''}
          ${exp ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">● Experience</div>${exp.split('\n').filter(Boolean).map(line=>`<div style="padding:8px 0 8px 14px;border-left:2px solid ${acc}55;margin-bottom:8px;font-size:0.83rem;color:#ccc;">${line}</div>`).join('')}<div style="margin-bottom:18px;"></div>` : ''}
          ${edu ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">● Education</div><p style="font-size:0.83rem;line-height:1.8;color:#aaa;white-space:pre-wrap;margin-bottom:18px;">${edu}</p>` : ''}
          ${proj ? `<div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;">● Projects</div><p style="font-size:0.83rem;line-height:1.8;color:#aaa;white-space:pre-wrap;">${proj}</p>` : ''}
        </div>
      </div>
    </div>`;

  // ==================== ELEGANT — Light Sidebar Classic ====================
  } else if (tpl === 'elegant') {
    html = `
    <div style="${fontCSS}background:#fff;display:grid;grid-template-columns:210px 1fr;min-height:550px;">
      <div style="background:#f4f4f8;padding:32px 22px;border-right:1px solid #e0e0ea;display:flex;flex-direction:column;gap:0;">
        <div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,${acc},${acc}88);display:flex;align-items:center;justify-content:center;font-size:1.8rem;font-weight:700;color:#fff;margin:0 auto 18px;"></div>
        <div style="text-align:center;margin-bottom:20px;">
          <div style="font-size:1.1rem;font-weight:700;color:#1a1a2e;">${name}</div>
          <div style="font-size:0.72rem;color:${acc};margin-top:5px;letter-spacing:1px;">${title}</div>
        </div>
        <div style="height:1px;background:#ddd;margin-bottom:16px;"></div>
        <div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:#999;margin-bottom:10px;">Contact</div>
        ${contactParts.map(c=>`<div style="font-size:0.76rem;color:#555;margin-bottom:7px;line-height:1.5;">▸ ${c}</div>`).join('')}
        ${socialLinks ? `<div style="font-size:0.7rem;color:#888;margin-top:6px;word-break:break-all;">${socialLinks}</div>` : ''}
        ${skills ? `<div style="height:1px;background:#ddd;margin:16px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:#999;margin-bottom:12px;">Skills</div>${skills.split(',').map(s=>`<div style="padding:5px 0;border-bottom:1px solid #ebebf2;font-size:0.78rem;color:#444;display:flex;align-items:center;gap:6px;"><span style="color:${acc};">◆</span>${s.trim()}</div>`).join('')}` : ''}
        ${lang ? `<div style="height:1px;background:#ddd;margin:16px 0;"></div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:3px;color:#999;margin-bottom:10px;">Languages</div>${lang.split(',').map(l=>`<div style="font-size:0.78rem;color:#555;margin-bottom:5px;">◈ ${l.trim()}</div>`).join('')}` : ''}
      </div>
      <div style="padding:32px 36px;background:#fff;">
        ${about ? `<div style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid ${acc}33;">Profile Summary</div><p style="font-size:0.87rem;line-height:1.8;color:#444;margin-bottom:24px;font-style:italic;">${about}</p>` : ''}
        ${exp ? `<div style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:6px;border-bottom:2px solid ${acc}33;">Work Experience</div>${exp.split('\n').filter(Boolean).map(line=>`<div style="padding:8px 0;border-bottom:1px solid #f5f5f5;font-size:0.85rem;color:#333;">${line}</div>`).join('')}<div style="margin-bottom:22px;"></div>` : ''}
        ${edu ? `<div style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:6px;border-bottom:2px solid ${acc}33;">Education</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:22px;">${edu}</p>` : ''}
        ${proj ? `<div style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:${acc};margin-bottom:12px;padding-bottom:6px;border-bottom:2px solid ${acc}33;">Projects</div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
      </div>
    </div>`;

  // ==================== COLORFUL — Bright Modern ====================
  } else if (tpl === 'colorful') {
    const colors = ['#ff6b9d','#ffd60a','#30d158','#00e5ff','#bf5af2','#ff9f43'];
    html = `
    <div style="${fontCSS}background:#fff;color:#1a1a2e;min-height:550px;">
      <div style="height:8px;background:linear-gradient(90deg,#ff6b9d,#ffd60a,#30d158,#00e5ff,#bf5af2);"></div>
      <div style="padding:32px 40px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px;">
        <div>
          <div style="font-size:2.2rem;font-weight:900;color:#0a0a1a;letter-spacing:-1px;">${name}</div>
          <div style="font-size:0.9rem;color:${acc};margin-top:5px;font-weight:600;letter-spacing:1px;">${title}</div>
        </div>
        <div style="text-align:right;">
          ${contactParts.map(c=>`<div style="font-size:0.76rem;color:#666;margin-bottom:4px;">${c}</div>`).join('')}
          ${socialLinks ? `<div style="font-size:0.72rem;color:#888;margin-top:4px;">${socialLinks}</div>` : ''}
        </div>
      </div>
      <div style="padding:28px 40px;">
        ${about ? `<div style="background:${acc}12;border-left:4px solid ${acc};padding:14px 18px;border-radius:0 8px 8px 0;margin-bottom:24px;"><p style="font-size:0.87rem;line-height:1.8;color:#333;margin:0;">${about}</p></div>` : ''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;">
          <div>
            ${skills ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><div style="width:16px;height:16px;border-radius:4px;background:${colors[0]};flex-shrink:0;"></div><span style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#333;">Skills</span></div><div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:24px;">${skills.split(',').map((s,i)=>`<span style="background:${colors[i%colors.length]}22;border:1px solid ${colors[i%colors.length]}66;color:#1a1a2e;padding:4px 12px;border-radius:20px;font-size:0.78rem;font-weight:500;">${s.trim()}</span>`).join('')}</div>` : ''}
            ${lang ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><div style="width:16px;height:16px;border-radius:4px;background:${colors[2]};flex-shrink:0;"></div><span style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#333;">Languages</span></div>${lang.split(',').map(l=>`<div style="padding:5px 0;font-size:0.83rem;color:#444;border-bottom:1px solid #f0f0f0;">◈ ${l.trim()}</div>`).join('')}` : ''}
          </div>
          <div>
            ${exp ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><div style="width:16px;height:16px;border-radius:4px;background:${colors[1]};flex-shrink:0;"></div><span style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#333;">Experience</span></div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:24px;">${exp}</p>` : ''}
            ${edu ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><div style="width:16px;height:16px;border-radius:4px;background:${colors[3]};flex-shrink:0;"></div><span style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#333;">Education</span></div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;margin-bottom:24px;">${edu}</p>` : ''}
            ${proj ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><div style="width:16px;height:16px;border-radius:4px;background:${colors[4]};flex-shrink:0;"></div><span style="font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#333;">Projects</span></div><p style="font-size:0.85rem;line-height:1.8;color:#333;white-space:pre-wrap;">${proj}</p>` : ''}
          </div>
        </div>
      </div>
    </div>`;
